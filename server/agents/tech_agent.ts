...
/**
 * Autonomous Agent for Tech Markets
 */
export class TechAgent {
    constructor() {
        eventBus.on(SystemEvents.SIGNAL_RECEIVED, async (signal: NormalizedSignal) => {
            if (signal.category === 'tech') {
                Logger.info(`[TECH_AGENT] Fetching & processing tech signal...`);
                ...
                if (proposal.confidence > 0.7) {
                    ...
                    Logger.warn(`[TECH_AGENT] Proposal rejected. Confidence ${proposal.confidence} <= 0.7`);
                }
            }
        });
    }
